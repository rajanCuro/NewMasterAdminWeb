  ...
  const getTravelPath = async () => {
    try {
      const response = await axiosInstance.get(
        `/api/field-executive/getTravelPathByDate?userId=${data?.id}&date=${chooseDate}`
      );
      ...
      <div className="w-full lg:w-1/4 xl:w-1/4 flex flex-col gap-3 p-3 md:p-4 border border-gray-300 rounded-2xl bg-white shadow-sm">
        <h2 className="text-lg md:text-xl font-semibold text-gray-800 mb-2">
          ...
          {firstName && lastName && (
            <span className="block text-sm text-gray-600 mt-1">
              for {firstName} {lastName} - {chooseDate}
            </span>
          )}
        </h2>
        ...
        <div className="flex items-center gap-3 p-3 md:p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div className="flex flex-col">
            ...
            <span className="flex items-center justify-center gap-1 text-lg md:text-xl font-bold text-blue-700">
              <input
                type="date"
                value={chooseDate}
                onChange={(e) => {
              ...
              <button
                onClick={getTravelPath}
                className="ml-1 px-2 py-1 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition text-md hover:shadow-lg cursor-pointer"
              >
                Go
              </button>
            </span>
        ...
        <div className="mt-2 p-3 md:p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <div className="text-xs md:text-sm text-gray-600 space-y-1">
            <p>
              Avg. distance/visit:{" "}
              {totalVisits > 0
                ? (totalTravelDistance / totalVisits).toFixed(2)
                : 0}{" "}
              km
            </p>
            <p>
              Completion rate:{" "}
              {totalVisits > 0
                ? ((totalCompletedTasks / totalVisits) * 100).toFixed(1)
                : 0}
              %
            </p>
          </div>
